...
## Hard-Won Lessons (SOWs 0093, 0094, 0095, 0117, 0118)
...
- **SOW-0118 cursor O(n) trap**: a value-receiver method that clones a map (e.g. `Cursor.withFile` used `maps.Copy`) is O(map_size) PER CALL. At 325K files with an average map of 162K entries, that's ~52 billion map copies — the dominant single-core cost (38% of CPU). Rule: in hot paths that run per-element (per file, per event, per row), NEVER use value-receiver methods that clone maps or slices. Use pointer receivers and mutate in-place. Profile the real system at scale — fresh-DB benchmarks hide this because empty maps are cheap to copy.
- **SOW-0118 coalescer pattern**: with `SetMaxOpenConns(1)` (SQLite single-writer), N independent workers each calling `BeginTx` independently serialize through the connection pool — 80%+ of their time is begin-wait (waiting for another worker's flush to finish). A single coalescer goroutine that owns the connection and drains all sources through a merged channel eliminates the begin-wait entirely. Per-flush time drops from 77ms to 0.7ms (110×). This is the architectural fix for single-writer contention; it does NOT add parallel writers.
- **SOW-0118 prepare-statement reuse**: modernc/database/sql re-prepares the SQL on every `ExecContext(ctx, sqlString, args)` — statement preparation was 59% of the `apply` cost. Cache prepared statements per-tx (writer.beginTx/endTx lifecycle, txExec helper that lazy-prepares+reuses). Cuts per-event overhead by ~10×.
- **SOW-0118 resolver busy-loop at scale**: the resolver's `linkRoots` recursive CTE is O(sessions) — ~30s on 552K rows at 31GB. At a 5s resolver interval, each pass takes longer than the interval, so the resolver NEVER catches up — it burns 60%+ of CPU indefinitely. Fix: gate the recursive CTE on `lastRootsGen` (run only once per generation advance, not once per 5s tick). `linkParents` (index-backed, fast) still runs on every sessionsChanged pass.
- **SOW-0118 resolver startup race**: `Ingester.Start` launches the resolver goroutine BEFORE `main.go` calls `SetStartupScanActive(true)`. The resolver's first tick fires with `startupScanActive=false` (the zero value) and runs its O(sessions) pass on the large DB, monopolizing the connection for minutes. Fix: initialize `startupScanActive` to TRUE in the Ingester struct so the resolver is deferred from its very first tick.
- **SOW-0118 read-model defer during scan**: the resolver, idle rollup refresh, and per-source read-model repair all do expensive DB work that is redundant during the initial scan (rebuilt post-scan by BackfillReadModels). Deferring them while `startupScanActive` is true eliminates the connection monopolization that previously caused begin-wait 68–93% during scans.
- **SOW-0118 index-drop lifecycle**: for the INITIAL scan only, dropping the 42 non-unique secondary indexes eliminates ~95% of per-event write amplification (the dominant cost at GB scale). The 2 UNIQUE constraints (upsert ON CONFLICT targets) MUST be kept — dropping them is a DATA-LOSS FOOTGUN (the naive `DROP INDEX` broke the `payload_refs` ON CONFLICT and silently dropped events). Use `DropNonUniqueIndexes` (which keeps all UNIQUE + PK) and `RecreateIndexes` (rebuilds from captured DDL post-scan). Tail/incremental MUST keep all indexes (serve needs them).
- **SOW-0118 NEVER experiment on the live production DB**: clearing the cursor or dropping indexes on the running daemon caused a data-loss incident (89 events dropped) and left the DB missing 21 indexes. All index/scan experiments must use throwaway DB copies or populated benchmark DBs. The constraint is: the live DB is for running the tested binary, not for experiments.
- **SOW-0118 long sessions are legitimate**: 535MB / 60K-event session files are the future of mature AI agents (weeks-long Codex sessions, multi-day sessions). NEVER cap/tame the data. The system must handle arbitrarily large sessions efficiently. The cursor O(1) fix and the coalescer are what make this possible at scale.
- **SOW-0117 json_extract index rule**: a `WHERE json_extract(col, '$.path')` predicate can use NO b-tree index and full-scans + JSON-parses every candidate row. Back it with a PARTIAL EXPRESSION index: `CREATE INDEX … ON t(json_extract(col,'$.path')) WHERE json_extract(col,'$.path') IS NOT NULL` — only the few stashed rows are indexed, so the query seeks them. On the 5 M-op store the resolver's two ops passes dropped 1.04 s/1.49 s → 0.3 s/6 ms. Expression-column entries report as `""` in PRAGMA index_info (precedent: `idx_sessions_tokens`).
- **SOW-0117 cadence-gate rule**: a periodic maintenance loop that does O(all-rows) work every tick is an unconditional CPU burn, even when idle. Gate it on a cheap "did anything change?" signal: (1) an in-memory atomic generation counter bumped on every commit (skip the whole pass when unchanged), and (2) for sub-passes scoped to one table, a `MAX(<monotonic ts>)` covering-index probe (skip the sub-pass when that table didn't change). Verify the ts is monotonic (all writes use `MAX(existing, new)`). Advance the watermark only on a SUCCESSFUL pass so a transient failure retries next tick. Measured: resolver CPU share during active scan 73% → 2.17%, idle → ~0.
- **SOW-0117 recursive-CTE gate**: `linkRoots` builds an O(sessions) recursive CTE every pass. It CANNOT be gated on "did linkParents change something" — the writer sets a PROVISIONAL direct-parent `root_session_id` at insert that the CTE must transitively correct even when no parent linked this pass (see `TestResolver_PropagatesNestedDirectParentRoot`). Gate it on the session table changing instead (the cadence-gate rule above), which is the correct signal.
