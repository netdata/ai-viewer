...
- **Adapter discovery probes** — `format: "<name>"` probe structs in `cmd/ai-viewer-ingest/source_discovery.go` vs. the matching `.agents/sow/specs/adapter-<name>.md` (underscore→hyphen) and that spec mentioning the default probe path.
