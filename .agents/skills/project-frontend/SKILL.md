...
- **Fail-closed on manifest-contract violations:** the gate exits non-zero on a missing/empty dist, a missing/invalid manifest (incl. a JSON array), zero classified JS chunks, no main (`isEntry`) chunk, a manifest `.file` absent on disk, a static-import graph referencing a missing/invalid chunk key, an `imports`/`dynamicImports` that is present but **not an array**, or an `imports` array holding a **non-string** element. It never certifies "within budget" without measuring real, classified chunks.
