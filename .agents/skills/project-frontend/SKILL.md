...
> **Local aggregate runner.** The repo-wide `scripts/lint.sh` is the build-free static-analysis entrypoint: its frontend section runs `npm run lint -- --max-warnings 0`, `npm run typecheck`, and the two hermetic gate self-tests (`check:bundle-size:selftest`, `check:coverage-thresholds:selftest`), fail-fast, after the Go gates. It ensures `frontend/node_modules` is present (reusing `build.sh`'s `npm ci`/`npm install` fallback, only when missing) so the self-tests find their binaries. It does NOT run the REAL bundle-size gate (`npm run check:bundle-size`, needs a `vite build`) or the REAL coverage run (`npm run test -- --run --coverage`) — those need a build/full test run and stay in the CI `frontend` job + `scripts/{build,test}.sh`.
