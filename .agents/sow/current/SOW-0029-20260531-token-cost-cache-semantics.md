...
- **P1 (codex + CI gate): PII cleanup incomplete (BLOCKER).** Round-2's grep was case-SENSITIVE, so it missed the operator name "Costa" at `SOW-0028:20` ("Costa's standing principle"); and the round-2 Reviews note ITSELF (`SOW-0029:128`) re-introduced the session-id fragments while describing the fix and falsely claimed the grep was clean. CI `gates` job was RED (scan-secrets flagged both as `[operator-name]`). **Fixed:** SOW-0028:20 → "the operator's"; SOW-0029:128 rewritten without the name/ids/path + corrected the false claim; also sanitized a real session UUID in `adapter-aiagent-v2.md:214` → `<parent-traceId>`. **Verified:** `scripts/scan-secrets.sh` now PASS (660 files); case-INSENSITIVE `-i` sweep clean. (Lesson recorded: grep `-i`, and re-grep AFTER editing review notes, before every commit.)
