...
Post-fix (orchestrator, ground truth): `go test -race` green on ingest/store/presenter; golangci-lint 0; gosec 0.

### Round 2 — 2026-05-31 (codex + glm + minimax, parallel) on commit `213dab9`

glm + minimax: CLEAN (no P1/P2; round-1 fixes verified). codex (decisive) found 2 more P2 + 2 P3, each adjudicated against code:

- **P2 (codex) — serve could serve stale durations against a pre-0005 DB.** Verified real: `ai-viewer-serve` runs NO migrations (`store.go` OpenReader) and gates startup ONLY on `schema_meta.version` (`CheckSchema`, exact-equality). The round-1 migration left the version at `'4'` (I had made it version-neutral), so serve could not detect 0005 and would hand out stale `duration_us=0` until the ingester migrated. **DECISION REVERSED: 0005 now bumps `schema_meta.version`→`'5'` and `presenter.SchemaVersion`→5**, so serve refuses a pre-0005 store via the existing mechanism. Verified safe — a version bump has no re-ingest/cursor-reset side effect (the ingester never reads `schema_meta`). Swept all version assertions to 5 (presenter const, 0004/store tests); `embed-smoke.sh` now waits for `0005_op_duration_backfill.sql`; embed-smoke re-run passes with `schema_meta.version = 5`. (0005 bumps because serve reads the durations it repairs; the rule is "bump when serve depends on the migration's outcome" — `0002_source_progress` is ingester-only and stays version-neutral.)
- **P2 (codex) — `start_ts = MIN(...)` can lower start_ts post-finalize without recomputing duration.** Verified real but PRE-EXISTING (the old finalize-time computation had the same property) and NOT triggered by real adapters (they re-emit the same/later start, never an earlier one post-finalize). **Documented the contract** (`ingester.md`: duration is computed at finalize from the then-persisted start_ts; adapters emit the authoritative start at/before finalize) and **filed follow-up SOW-0027** for a defensive recompute / generated-column. Not a SOW-0026 regression; not a merge blocker.
- **P3 (codex) — migration positive-ts gate parity.** Fixed: backfill WHERE adds `start_ts > 0 AND end_ts > 0` (exact writer-gate parity).
