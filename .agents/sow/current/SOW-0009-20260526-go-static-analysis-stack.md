...
4. govulncheck runs **per push (blocking) AND on a nightly schedule** (the per-push job blocks but, being call-graph aware, fails only on reachable vulns, so it does not flap on unreachable transitive CVEs; the nightly job additionally sweeps master so a CVE disclosed between pushes surfaces within a day). **Verification**: `ci.yml` keeps the per-push govulncheck; a scheduled workflow (`on.schedule`) runs it nightly; both visible via `gh workflow view`.
...
- **R2 — govulncheck on fresh CVEs**: govulncheck is call-graph aware — it fails only when the code actually calls a vulnerable symbol, so unreachable transitive CVEs do not flap the per-push (blocking) job. The nightly job is an additional scheduled sweep so a CVE disclosed against master between pushes surfaces within a day. Two jobs (both blocking; not `continue-on-error`).
...
### Round 3 — 2026-06-02 (re-review on commit db9ebf2)

- **glm** (`glm-5.1`): **MERGE.** Round-3 fixes complete for the surfaces SOW-0009 touched; GOBIN fix correct. Flagged AGENTS.md:122 / :260-262 / :351-353 — all addressed in round 4 (glm reviewed the pre-round-4 commit).
- **codex** (`gpt-5.5`, decisive): runtime mergeable (GOBIN verified correct; all runtime re-checks pass). **BLOCKED on 3 findings, all adjudicated REAL** — the doc sweep was STILL incomplete (I again fixed cited lines, not the whole class; the script I kept missing was `spec-drift.sh`):
  - **P1** (doc drift): **Round-4 fix = exhaustive grep of all THREE planned scripts (`test.sh`/`gates.sh`/`spec-drift.sh`) across every durable doc, then caveat EVERY mention**: AGENTS.md:122 ("added in Phase 1" → planned SOW-0013) + the repo-layout tree, specs/index.md:59, project-specs-sync:76 (reconciled with its own line 88), quality-gates.md:185, project-testing:137, the project-quality-gates Spec-Drift block + :252/:261. A post-fix grep confirms every mention is now caveated.
  - **P2** (nolint contradiction): project-coding:77 forbade reason-only `//nolint`, contradicting the refined `project-quality-gates` policy the PR's nilerr suppressions rely on. **Round-4 fix** = reconciled project-coding:77 to the refined policy (permanent-architectural needs a reason; deferred-fix needs a link).
  - **P3** (govulncheck wording): the nightly workflow called the per-push job "advisory", but ci.yml runs it blocking. **Round-4 fix** = corrected the framing everywhere (per-push BLOCKS but, being call-graph aware, fails only on reachable vulns so it won't flap; the nightly is an additional between-push sweep) — nightly comment + SOW R2 + AC#4.
- **minimax** (`MiniMax-M3`): round-3 verdict folded into the round-4 re-review.
