...
4. **Race cadence — per-push stays `-count=1`; stress is nightly.** The `internal/ingest` suite includes a slow test (`TestRefreshRollups_OtherStaleRowRemoval` — several minutes under `-race`, grown since SOW-0009's ~240s estimate); `-count=3` on every push would balloon the test job and hurt PR feedback latency for marginal added race-coverage. Keep per-push `-count=1` (current); add a **nightly** scheduled race-stress job (`-count=10`); document the local `--stress` flag (`-count=10`). Higher per-push counts are gated on first speeding up that test (SOW-0009 Followup).
...
Land the Go test execution + statement-coverage threshold enforcement layer. **As delivered** (re-scoped 2026-06-02 — see the Re-scope decisions above): every push runs the full Go test suite with the race detector and atomic **statement**-coverage counters, and a build-failing CI gate (`scripts/check-coverage.sh`) fails the push if any gated `internal/*` package or the gated aggregate is below 80% statements (`/cmd/` excluded). A nightly scheduled job runs `-count=10` race stress. Coverage is published as a CI artifact.

> **Provenance note:** the rest of this narrative (Requirements, Assistant Understanding, Analysis, Pre-Implementation Gate, Plan) is the original 2026-05-26 draft. Where it references the *four*-threshold gate, line-coverage, PR-diff / new-code coverage, a `--diff` mode, `-count=3` per-push / `-count=20` nightly, PR sticky comments, the README badge, or `scripts/gates.sh` integration, those are **SUPERSEDED** by the Re-scope decisions above and the re-scoped Acceptance Criteria below. The Execution Log records what actually shipped.
...
2. `scripts/check-coverage.sh` exists, executable, parses `coverage.out` (`go tool cover -func`) and enforces **statement** coverage: **the gated `internal/*` aggregate ≥ 80%** and **per-package ≥ 80% for every `internal/*` package**. A package is gated **iff** its path contains `/internal/` and not `/cmd/`: `cmd/*` is excluded (entrypoints + dev tools; covered by E2E/embed-smoke/cmd-binary tests), and so is any non-`internal/` path such as vendored Go a frontend npm dep ships under `frontend/node_modules/` — the predicate + rationale are in the script. Output is human-readable and actionable (names the failing package + the missing percentage). **Verification**: synthetic `coverage.out` fixtures with known per-package percentages exercise pass + each below-threshold miss; the script exits non-zero on each miss, zero when all pass; a run on the current tree exits 0 (all `internal/*` ≥ 80%).
...
- Delivered (master-owned scripts/CI/specs — no production `.go` change):
  ...
  - `scripts/check-coverage.sh` — statement gate: a package is gated **iff** its path contains `/internal/` and not `/cmd/`; each gated package + their aggregate ≥ 80%. Excluded: `/cmd/` (top-level binaries + nested `internal/.../cmd/` dev-tools) and non-`internal/` paths — e.g. vendored Go a frontend npm dep ships under `frontend/node_modules/flatted/golang` (`go test ./...` covers it; not our code). [The `/internal/`-positive predicate replaced an initial `/cmd/`-only exclusion after external review round 3 found the latter spuriously gated `flatted` at 0% whenever node_modules was present locally — CI passed only because node_modules is absent during CI's `go test`. This is the SOW's own R5 "local-vs-CI divergence" risk, materialized + closed.] Validated on a full live profile (2026-06-02): all 13 gated `internal/*` packages 84–100%, gated aggregate 90.4% (8961/9908 stmts), PASS. (An earlier *partial* `coverage.out` — missing `ingest`/`presenter`/`aiagent_v2` — showed 10 packages / 91.1%; superseded by this full-tree measurement.)
  - `scripts/test/check-coverage-test.sh` — synthetic-fixture self-tests (pass / gated-miss / cmd-excluded / aggregate-miss / aggregate-fail-line-fires / non-internal-vendored-Go-excluded / missing-profile); 8/8 assertions pass.
  - `ci.yml`: a "Self-test coverage gate" step (runs `check-coverage-test.sh` so the gate script is regression-protected in CI, not only locally) + the "Enforce coverage thresholds" build-failing step in the `test` job (after the artifact upload).
...
- **Per-push CI should invoke `scripts/test.sh`** → the `test` job inlines `go test -race -coverprofile=coverage.out -covermode=atomic -count=1 ./...` rather than calling `scripts/test.sh` (the spec's single-source-of-truth entrypoint; the nightly stress job already calls it). Pre-existing (predates this SOW); a future edit to `test.sh` would not propagate to per-push CI — the SOW's own R5 "local-vs-CI divergence" class. Fold into SOW-0013's CI wiring or a focused follow-up. (Surfaced in SOW-0010 review round 5.)
- **embed-smoke seed marker** (regression, fixed in this SOW per operator decision): `embed-smoke.sh` hardcoded a `0005` seed marker that drifted when SOW-0007 added migrations 0006/0007 → flaky v5 seed. Now derives the last migration dynamically + pinned by `scripts/test/embed-smoke-seed-marker-test.sh` (CI-wired). Recorded as a SOW-0007 Regression.
