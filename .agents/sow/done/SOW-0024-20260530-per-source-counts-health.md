...
- **A very-slow test on master silently inflates every gate that runs `go test ./...`.** The pre-existing `TestRefreshRollups_OtherStaleRowRemoval` (+ `ParityWithBackfill`) took ~242s + ~120s under `-race` (writing 2,100 real events to exceed a hardcoded 2,000 collapse threshold). I misread a truncated goroutine dump during SOW-0024 and mis-filed it as a "hang/deadlock" (SOW-0062) — it was never a deadlock, just slow; the default 10m timeout always passed. Corrected and fixed in SOW-0062 (injectable threshold → both tests now run in ~1.3s; full ingest package 371.8s→23.1s under `-race`). Lesson: a goroutine shown as `[chan receive]` in `(*Tx).awaitDone` is the NORMAL per-tx context-watcher, not the blocker — read the FULL dump and check the test goroutine's state (`[runnable]` = executing, not blocked) before claiming a deadlock.
