...
3. **Trace (APM)** — the primary "what happened, in what order, how long" view (SOW-0006 decision). **Whole-tree (SOW-0070):** the trace spans EVERY session in the tree (root + sub-agents), fetched from `GET /api/sessions/:id/trace`; each op is tagged with its owning session so the client builds ONE merged op tree — a sub-session's ops nest beneath the `child_session_id` op that spawned them. A per-row sub-agent indicator (color keyed by `session_agent_name`) makes which sub-agent executed each op obvious at a glance. Toggleable renderings of the same merged op tree, plus a list:
   ...
   - **Filters (SOW-0070):** op kind (all/llm/tool/session/reasoning/…), status (all/completed/failed), and **sub-agent** (all/`<agent_name>`), narrowing the merged trace.
