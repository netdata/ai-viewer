...
**It runs only on the precise signature of the invisible update** — a WAL-DRIVEN probe on which NEITHER detector advanced (`probed && lastWALEvent.After(lastProbe) && !changed`):
- `lastWALEvent.After(lastProbe)` — a real opencode write fired the `opencode.db-wal` fsnotify hint since the last probe (not the periodic 60 s safety-net tick of a stable DB).
- `!changed` — yet neither the cheap `MAX(id)` (no insert) nor the gated `MAX(time_updated) >` (boundary ms unchanged) saw any advance. An INSERT advances `MAX(id)` → `changed == true` → the forward delta (`processChanges`) handles it and the boundary re-scan is SKIPPED.

This precision is what reconciles the fix with the existing contracts:
- A steady-state IDLE DB (no writes → no WAL event) re-emits nothing, even when the 60 s net opens the gate. AC#6's zero-expensive-query idle property is untouched (the re-scan is on the gated path; the cheap `MAX(id)` idle path is unchanged — strict `>` against the monotonic `MaxIDSeen`); the round-2 counting-driver / `TestP1A_OldRowUpdateDoesNotReArmIdleScan` tests stay green.
- A **cold `Tail` HEAD snapshot** never replays its boundary session: the only writes that follow a cold snapshot are INSERTs (which advance `MAX(id)` → `changed == true` → boundary skipped), so a session entirely at-or-below the snapshot watermark is never re-emitted.
- A genuine in-place boundary update (opencode re-stamps a row's `time_updated` to the same ms → WAL mtime changes, but no id/max-time advance) hits exactly `walDriven && !changed` and IS caught.
