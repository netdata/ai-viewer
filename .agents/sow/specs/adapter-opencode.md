...
1. **Paged delta query per tracked table.** Each `session`/`message`/`part`/`session_message` table is paged from its `TableWatermark` with the composite-key SELECT (`buildSelect`, naming only live columns — never `SELECT *`):
   ...
   **Corrupt delta-row cells (SOW-0005 round-4 P2-1).** The per-table delta scanners carry the same `onWarn` hook the non-delta `loadSession` path uses, so a corrupt OPTIONAL numeric cell (a non-NULL value unparseable as its column's numeric type — e.g. `cost`, `tokens_*`, `time_created`) surfaces a structured WARN with table/column context and degrades to 0 rather than being silently coerced. The two REQUIRED cursor-watermark columns (`id`, `time_updated`) are treated more strictly: a corrupt/absent/NULL value there returns an ERROR for that row (`i64Required`/`strRequired` via `requiredWatermark`), which aborts the delta page so the cursor is NEVER advanced to a poisoned watermark (a `time_updated` coerced to 0 could regress the watermark). The error is non-fatal to the loop — it is surfaced via `onError` and the cursor stays at the last good position. The boundary re-scan shares the same scanners and therefore the same guard.
...
**It runs on a gate-open probe on which NEITHER detector advanced** (`probed && !changed`) under EITHER trigger (SOW-0005 round-4 P1 completed round-3's partial fix):
...
In both cases `!changed` means neither the cheap `MAX(id)` (no insert) nor the gated `MAX(time_updated) >` (boundary ms unchanged) saw any advance. An INSERT advances `MAX(id)` → `changed == true` → the forward delta (`processChanges`) handles it and the boundary re-scan is SKIPPED.
...
- A **cold `Tail` HEAD snapshot** never replays its boundary session on its first probe (the `priorProbe` guard), and thereafter the only writes that follow a cold snapshot are INSERTs (which advance `MAX(id)` → `changed == true` → boundary skipped), so a session entirely at-or-below the snapshot watermark is never re-emitted.
- A genuine in-place boundary update (opencode re-stamps a row's `time_updated` to the same ms → WAL mtime changes, but no id/max-time advance) hits `walDriven && !changed` and IS caught immediately; if the WAL hint is missed it is caught on the next safety-net probe (`priorProbe && !changed`).
