...
**It runs on EVERY gate-open probe** (`probed`) under EITHER trigger (SOW-0005 round-4 P1 completed round-3's partial fix; round-6 P1 made it run regardless of `changed`):
- **WAL-driven** (`lastWALEvent.After(lastProbe)`) — a real opencode write fired the `opencode.db-wal` fsnotify hint since the last probe. The immediate path (round-3).
- **Safety-net** (`priorProbe`) — the 60 s net opened the gate AND at least one probe has already completed in this tailer's life. This catches the same-ms in-place update when the WAL hint is **missed** (a dropped fsnotify event, a watcher whose `Add` failed, or timer-only polling): the spec's safety net guarantees in-place mutations are eventually seen EVEN IF the WAL hint never fires, so the boundary re-scan must run on the net path too, not only the WAL path.

**Ordering — boundary re-scan BEFORE the forward delta, against the PRE-ADVANCE cursor (SOW-0005 round-6 P1).** The re-scan runs FIRST in `pollOnce` — against the cursor's *current* (pre-advance) `MaxTimeUpdatedMs == T` — and ONLY THEN does `processChanges` page the forward delta (which advances the cursor). The round-3/4 code ran the re-scan only on the `changed == false` branch, which stranded a low-id in-place update at boundary ms `T` (row A) whenever a **co-occurring NORMAL forward change** (row B — e.g. another row in the same table whose `time_updated` was bumped to `T2 > T`) made `detectChange` return `changed == true`: the forward delta advanced the cursor PAST `T`, and row A — excluded from that delta by the strict tie-break `id > highID` — fell permanently below the new watermark, never seen (a zero-gaps violation). Running the bounded boundary-bucket re-scan against the pre-advance `T` BEFORE the cursor moves re-emits row A's session first; the forward delta then emits row B and advances to `T2`. A session caught by BOTH paths is re-emitted once per path and absorbed idempotently by the writer, with no churn beyond the (tiny) boundary bucket. The boundary re-scan only ever runs on a *probed* cycle (a `MAX(time_updated)` probe ran).

**`boundaryReal` — the changed==true path is gated so the cold-`Tail` HEAD-snapshot contract survives the reorder.** When `changed == true`, the pre-advance re-scan runs ONLY if `boundaryReal` is set; when `changed == false` it always runs (no forward advance can strand it, and the `priorProbe` cold guard below already prevents a pristine cold first probe from replaying). `boundaryReal` means the cursor's boundary `T` is a position whose bucket was ALREADY emitted, so re-scanning it is idempotent rather than a cold replay. It starts **true for a WARM `Tail`** (resumed from a Scan cursor — Scan already emitted the boundary) and **false for a COLD `Tail`** (HEAD snapshot, follow-from-now: the boundary bucket was never emitted); `pollOnce` flips it true once the cursor first advances (the new boundary is the just-emitted forward position). Without this gate, a cold `Tail` would replay a pre-existing session on the first post-snapshot forward change whose new row id sorts BELOW the snapshot `MAX(id)` — that change does NOT trip the cheap `MAX(id)` path (the id is smaller), so it trips the gated `MAX(time_updated)` probe instead (`changed == true, probed == true`), and an unconditional pre-advance re-scan against the snapshot boundary `T` would re-emit the snapshot's own boundary rows. (This is the exact `TestAdapter_TailColdSnapshot` regression the round-6 reorder first introduced and `boundaryReal` closes.)

The `priorProbe` flag is the **cold-`Tail` HEAD-snapshot replay guard**: it is read BEFORE `markProbe` sets it, so the VERY FIRST probe of a fresh `Tail` (a HEAD-snapshot reconciliation, where `priorProbe == false` and a HEAD snapshot is not WAL-driven) does NOT replay the snapshot's boundary session. Only a SUBSEQUENT safety-net probe (after a real cycle has run) may re-scan.

This precision is what reconciles the fix with the existing contracts:
- A steady-state IDLE DB **WITHIN the 60 s net** with no WAL event has `probed == false`, so the boundary re-scan never runs on an idle poll. AC#6's zero-expensive-query idle property is untouched (the re-scan is on the gated path; the cheap `MAX(id)` idle path is unchanged — strict `>` against the monotonic `MaxIDSeen`); the round-2 counting-driver / `TestP1A_OldRowUpdateDoesNotReArmIdleScan` tests stay green. The re-scan fires only when the gate OPENS — a WAL event or the 60 s net tick — which is exactly the safety net's purpose (a once-per-60 s bounded bucket re-emit on an otherwise-idle DB is the accepted cost of guaranteeing a missed-WAL in-place update is eventually surfaced).
- A **cold `Tail` HEAD snapshot** never replays its boundary session: the `priorProbe` guard blocks the first probe, and `boundaryReal == false` until the cursor first advances blocks the `changed == true` pre-advance re-scan (round-6 P1) — so even a post-snapshot forward change whose new row id sorts *below* the snapshot `MAX(id)` (which trips the `time_updated` probe rather than the cheap `MAX(id)` path) does NOT replay the snapshot boundary. Once the cursor advances past the snapshot, `boundaryReal` flips true and the boundary is a genuinely-emitted position thereafter. A WARM `Tail` (resumed from a Scan cursor) starts `boundaryReal == true` because Scan already emitted the boundary.
- A genuine in-place boundary update (opencode re-stamps a row's `time_updated` to the same ms → WAL mtime changes, but no id/max-time advance) is caught immediately on the WAL-driven probe (`walDriven`); if the WAL hint is missed it is caught on the next safety-net probe (`priorProbe`). The re-scan no longer requires `!changed` (round-6 P1): even when a co-occurring forward change makes `changed == true`, the re-scan still runs FIRST against the pre-advance boundary `T`, so the same-ms update is never stranded by the forward delta advancing the cursor.
