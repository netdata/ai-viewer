...
- **Latest migration / counts (AC#8).** `latest_migration` is the name with the highest `id` (last applied). The auto-discovery probe (`ProbeStatus`) reads it alongside `COUNT(*)` of `session`/`message`/`part`. Since SOW-0024 the probe result is PERSISTED into `sources.meta_json` as `{"session_count","message_count","part_count","latest_migration"}` (marshalled by `cmd/ai-viewer-ingest` auto-discovery and written by the ingester's `ensureSourceRow`), so it surfaces in `/api/health` and `/api/sources` under each source's optional `meta` field — not just the discovery log. The counts are the opencode source-DB native row counts (NOT ai-viewer's ingested canonical counts); freshness is the last ingester startup (the probe runs once at auto-discovery). File-based adapters do not populate the blob. A missing `__drizzle_migrations` table (a very old or foreign SQLite file) is non-fatal: the probe returns empty names + a soft sentinel, the schema hash is left empty, and no migration is reported — the adapter degrades rather than crashing.
...
2. **Token classes opencode tracks.** opencode separates `tokens.reasoning`, `tokens.cache.read`, `tokens.cache.write`. **Partially resolved:** `tokens_cache_read`/`tokens_cache_write` are now first-class canonical fields (SOW-0029 token contract) and the adapter maps them (`mapper_ops.go:149`, `mapper_turn.go:169`); cache reads materially affect cost (Anthropic and most others charge ~10% for cache reads vs full input price). **Remaining gap:** `tokens.reasoning` still has no canonical column — filed as a follow-up SOW.
