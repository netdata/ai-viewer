...
- Marked benchmarks for the 9 performance-critical paths: ai-agent v2 adapter `Scan` + adapter `Tail` (`internal/adapters/aiagent_v2`), claude-code adapter `Scan` + adapter `Tail` (`internal/adapters/claude_code`), Codex adapter `Scan` + adapter `Tail` (`internal/adapters/codex`), SQLite batch insert (`internal/ingest` `worker.flush`), REST query path (`internal/presenter` `handleSessionsList`), SSE fanout (`internal/notify` `Hub.Deliver`). There is **no canonical encode/decode** benchmark — canonical events are constructed directly by adapters and never serialized (`internal/canonical` has no encoders/decoders).
- `scripts/check-bench.sh` runs `go test -run=^$ -bench=. -benchmem -count=6` over the 6 benchmark packages (9 benchmarks; adapter `Scan` + `Tail` share `aiagent_v2`, claude-code adapter `Scan` + `Tail` share `claude_code`, and Codex adapter `Scan` + `Tail` share `codex`) and compares to `bench/baseline.txt` via `benchstat` (`-count=6` is benchstat's minimum for a 0.95 confidence interval).
