...
| `parentSessionId` | yes (sub-agents) | string | **A first-class producer guarantee since the lineage fix `ai-agent@8a0078bc`**: `recordSessionStart()` now always writes `parentSessionId` (and `parentOpId`) onto the child's `session_start` (`session-recorder.ts:364-366`), superseding the prior "present on ~76% of sessions / remaining ~3.2% early-format leftovers" observation. Set on every non-root session (sub-agent, `history_compaction`, `tool_output`); absent on root headends (`cli`, `api`, `web`, `embed`, `slack`) by definition. Pre-fix snapshots on disk may still omit it, so the adapter treats it as the immediate-parent linkage fast path and keeps the parent-side `childSessions[]` synthesizer (§8.1) as the two-path safety net. Maps to canonical `ParentNativeID` (`mapper.go:118`). |
| `parentOpId` | yes (sub-agents) | string | Added by the lineage fix `ai-agent@8a0078bc` and now written by `recordSessionStart()` alongside `parentSessionId` (`session-recorder.ts:364-366`); also carried on parent-side `childSessions[]` refs (`session-tree.ts`). Identifies the parent op that spawned the child. Maps to canonical `ParentOpKey` (`mapper.go:119`), which the ingester persists into `sessions.extras_json.aiViewer.parentOpKey`. The adapter MUST still tolerate its absence on pre-fix snapshots. |
...
1. **Child-side fast path** (preferred when present): `session_start.parentSessionId` (and `parentOpId`) is set on the child's own ledger. **Since the lineage fix `ai-agent@8a0078bc` this is a first-class producer guarantee** — `recordSessionStart()` always writes both on every non-root session (the fields come from `SessionEvidenceRecorderOptions.parentOpId`/`parentSessionId`, `session-recorder.ts:28-29`, and are emitted at `session-recorder.ts:364-366`). When the adapter sees this, it immediately emits `SessionStartedEvent.ParentNativeID = parentSessionId` (and `ParentOpKey = parentOpId`) and relies on the ingester to attach. **No parent file required.**
...
The ai-agent SOW `evidence-explicit-parent-id-on-child` **landed as `ai-agent@8a0078bc`** ("Add session lineage and LLM request id evidence"): `recordSessionStart()` now writes both **`parentOpId`** and `parentSessionId` on the child's own `session_start` (`session-recorder.ts:364-366`), so the child→parent-op linkage is resolvable from the child alone, without depending on the parent's `turn_end` for op-level attribution. The v3 adapter consumes both (`mapper.go:118-119`) and still tolerates their absence on pre-fix snapshots, preferring the explicit fields when present.
...
- `ai-agent.git/.agents/sow/done/SOW-0030-20260526-evidence-explicit-parent-id-on-child.md` — the (now completed) SOW that made `parentSessionId`/`parentOpId` first-class fields on `session_start`, shipped as `ai-agent@8a0078bc`
