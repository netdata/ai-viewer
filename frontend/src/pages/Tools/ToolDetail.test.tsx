...
function renderPage() {
  return render(
    <MemoryRouter initialEntries={['/tools/filesystem::read']}>
      <TooltipProvider>
        <Routes>
          <Route path="/tools" element={<div data-testid="tools-list" />} />
...
describe('ToolDetail', () => {
  ...
  it('renders the tool slug as the page title', () => {
    renderPage();
    expect(screen.getByRole('heading', { level: 1, name: 'filesystem::read' })).toBeInTheDocument();
  });

  it('binds the tool slug into the URL filter', () => {
    renderPage();
    expect(setFiltersSpy).toHaveBeenCalled();
    const lastCall = setFiltersSpy.mock.calls.at(-1)?.[0] as { tools?: string[] } | undefined;
    expect(lastCall?.tools).toEqual(['filesystem::read']);
  });

  it('renders a row per session', () => {
    const response: SessionListResponse = {
      items: [
        mkSession({ id: 'a', agent_name: 'agent-alpha', model: 'gpt-4o' }),
        mkSession({ id: 'b', agent_name: 'agent-beta', model: 'claude-3-5-sonnet' }),
      ],
    };
    sessionsSpy.mockReturnValue({
  ...
  it('shows the empty state when no sessions match', () => {
    ...
    renderPage();
    expect(screen.getByText(/No sessions called filesystem::read in this window/i)).toBeInTheDocument();
  });
});
