...
// Format is the stable identifier registered with the adapter registry and
// written into sources.format. The user-facing name is "claude-code"
// (hyphenated); the Go package directory uses the underscore form
// claude_code to keep grep-discoverability sharp and satisfy Go's import
// path rules.
const Format = "claude-code"

// sourceIDPrefix is prepended to the configured projects root to produce
// the canonical events' SourceID. Used only for log attribution;
// idempotency is a SQL-layer guarantee keyed on each table's natural
// identity (not SourceSeq).
const sourceIDPrefix = Format + ":"

// Adapter is the claude-code source adapter. One instance corresponds to
// one projects root (~/.claude/projects or $CLAUDE_CONFIG_DIR/projects).
// The instance is safe for a single Scan goroutine followed by a single
...
// New constructs an Adapter rooted at the given projects directory with the
// shared canonical.AdapterOptions bundle. An empty root is rejected so
// misconfigured ingesters fail fast.
func New(root string, opts canonical.AdapterOptions) (*Adapter, error) {
	if root == "" {
		return nil, errors.New("claude_code: root must be non-empty")
	}
	logger := opts.Logger
	if logger == nil {
		logger = slog.Default()
...
// Scan implements canonical.Adapter. Walks the projects root, reads each
// transcript (main + subagent) from its cursor offset to EOF, and emits
// canonical events. Returns when caught up or when ctx is cancelled. The
...
func (a *Adapter) Scan(ctx context.Context, since canonical.Cursor, out chan<- canonical.Event) error {
	...
	if sErr != nil {
		...
		return fmt.Errorf("claude_code: scan: %w", sErr)
	}
	return nil
}
...
// Cursor; non-empty input is decoded as JSON. The returned Cursor is opaque
// to the ingester and used only via Cursor.String() and Cursor.After().
...
// coerceCursor accepts a Cursor produced by this adapter, a nil Cursor
// (first run), or an alien cursor type (treated as empty so the ingester's
// "I lost track" path re-scans from offset 0). Never returns nil.
func (a *Adapter) coerceCursor(c canonical.Cursor) Cursor {
	...
	if typed, ok := c.(Cursor); ok {
		...
		if typed.MetaSeen == nil {
			typed.MetaSeen = map[string]string{}
		}
		if typed.Version == 0 {
...
// snapshotCursor builds a cursor from current on-disk transcript sizes so a
...
func (a *Adapter) snapshotCursor() (Cursor, error) {
	...
	for _, t := range transcripts {
		info, sErr := os.Stat(t.abs)
		if sErr != nil {
			a.onError(fmt.Errorf("claude_code: snapshot size %s: %w", t.rel, sErr))
			continue
		}
		size := info.Size()
		cur = cur.withFile(t.rel, FileCursor{Offset: size, Size: size})
	}
	return cur, nil
}

// Factory adapts New to canonical.AdapterFactory so the registry can
// construct an Adapter from the generic (location, opts) pair. The location
// is treated as the projects root.
