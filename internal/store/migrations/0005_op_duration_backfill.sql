...
-- durations of its member LLM ops. Grouping mirrors catalog.go exactly:
-- members are ops with kind='llm' whose (provider, model) equals the catalog
-- row's (provider, name). NULL durations (un-backfilled rows above) are
-- excluded; COALESCE keeps the NOT NULL column at 0 when a row has no members.
...
-- durations of its member tool ops. Grouping mirrors catalog.go exactly:
-- members are ops with kind='tool' whose normalized namespace
-- (COALESCE(NULLIF(tool_namespace,''),'builtin')) equals the catalog row's
-- namespace and whose name equals the catalog row's name.
