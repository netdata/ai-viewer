...
// schema_meta carries version='6' (the latest migration, 0006, bumps it).
...
func TestOpen_Idempotent(t *testing.T) {
	...
	for round := 1; round <= 3; round++ {
		...
		// REPLACE guard. Sanity-check the version is still '6'.
		var version string
		if err := s.DB().QueryRowContext(context.Background(),
			`SELECT value FROM schema_meta WHERE key='version'`).Scan(&version); err != nil {
		...
		if version != "6" {
			t.Fatalf("round %d: schema_meta.version: want %q, got %q", round, "6", version)
		}
